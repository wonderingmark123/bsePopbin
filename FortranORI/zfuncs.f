...
!----------------------------------------------------------------------
!
      real*8 FUNCTION ceebindf(kw,z,m0,m,mc,lum,rad,menv,fac)
      implicit none
      integer kw
      real*8 z,m0,m,mc,lum,rad,menv,fac
      real*8 menvd,rzams,lamb,logE
      real*8 rzamsf,celamf,calc_logBE
      external rzamsf,celamf,calc_logBE
      real*8 log10_solarunit_erg ! log10(G*Mo^2/Ro/erg)
      parameter(log10_solarunit_erg=48.579346d0)

      if(fac.gt.1.d0.and.kw.le.6)then
         logE = calc_logBE(kw,log10(z),m0,m,log10(rad))
         logE = logE - log10_solarunit_erg
	 if(logE.gt.100.) logE=100.
         ceebindf = 10.d0**logE
      !     write(*,*)'logE,ceebindf',logE,ceebindf
      else
         menvd = menv/(m-mc)
         rzams = rzamsf(m0)
         lamb = celamf(kw,m0,lum,rad,rzams,menvd,fac)
         ceebindf = m*(m-mc)/(lamb*rad)
      endif
