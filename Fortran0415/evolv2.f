...
*
      REAL*8 porb2,porb25,porb3,porb35,porb4,porb45,porb5,tb0
      REAL*8 porb6,porb8,porb10,porb12,q1,porb7,porb9,q2,tbr
...
*
     ...
     &                    /(2.d0*sep*sep*omv2)
         mkwgt=MAX(kstar(1),kstar(2))
         mkwlt=MIN(kstar(1),kstar(2))
         ! print *, kstar 
         
         fff= rad(k)/rol(k)*100
         qnow=q(k)
         vinf= SQRT(2*6.67*mass(k)*1.9891d+19
     &/(rad(k)*6.955d+08))
         vorb= (2*3.1416*(sep*6.955d+8/(tb*3.1557d+07)))
         nnn=vinf/vorb
         nnn=SQRT(2*6.67*mass(k)*1.9891d+19/
     &(rad(k)*6.955d+08))
     &/(2*3.1416*(sep*6.955d+8/(tb*3.1557d+07)))
         if(mkwgt.gt.12.and.mkwgt.lt.15.and.mkwlt.lt.10.and.
     &  mkwlt.gt.2)then
      ! .and.nnn.gt.1.0.and.nnn.lt.40.0
      ! if(mkwlt < 7)then
      !    nnn= SQRT(0.125) * nnn
      ! else
      !    if(mass(k) >120 )then
...
! 2020/11/08
      ...
      !    qc = 4.d0
         ...
         if(kstar(j1).le.1)then
            ...
            porb2 =0.674+0.2149*mass1i-0.009817*mass1i**2
     &  +0.000194*mass1i**3-1.365d-6*mass1i**4
   
   
           if(tbi.le.porb2)then
           qc =0.0
           elseif(tbi.gt.porb2.and.mass1i.le.25)then
             if(q1.le.2.0)then
             qc=1.0d4
             else
             qc=0.0
             endif
           elseif(tbi.gt.porb2.and.mass1i.le.35)then        
             if(q1.le.2.2)then
             qc=1.0d4
             else
             qc=0.0
             endif
           elseif(tbi.gt.porb2.and.mass1i.gt.35)then  
             if(q1.le.2.5)then
             qc=1.0d4
             else
             qc=0.0
         ...
         elseif(kstar(j1).eq.2)then   
             ...
             if(q1.le.1.8)then
             qc=1.0d4
             elseif(q1.ge.3.0)then
             qc=0.0
             elseif(q1.gt.1.8.and.q1.le.2.8)then
              if(mass1i.le.10.0.or.mass1i.ge.20.0)then
              qc = 1.0d4
              else
              qc = 0.0
              endif
             elseif(q1.gt.2.2.and.q1.le.2.5)then
              if(mass1i.le.7.0.or.mass1i.ge.45.0)then
              qc = 1.0d4
              else
              qc = 0.0
              endif   
             elseif(q1.gt.2.8.and.q1.lt.3.0)then
             if(mass1i.gt.45.0)then
              qc = 1.0d4
              else
              qc = 0.0
              endif   
...
*
     ...
     &        or.kstar(j1).eq.6.or.kstar(j1).eq.9).
...
*
     ...
     &               jspin(j2),kstar(j2),z,zpars,ecc,sep,jorb,coel)
*
         jp = MIN(80,jp + 1)
         bpp(jp,1) = tphys
         bpp(jp,2) = mass(1)
...
*
      ...
      bpp(jp,10) = 5.0
*
      if(kstar(j1).ge.3.and.kstar(j1).le.6.or.kstar(j1).eq.9)then
         CALL comenv(mass0(j1),mass(j1),massc(j1),aj(j1),jspin(j1),
     &               kstar(j1),mass0(j2),mass(j2),massc(j2),aj(j2),
     &               jspin(j2),kstar(j2),z,zpars,ecc,sep,jorb,coel)
         com = .true.
      elseif(kstar(j2).ge.3.and.kstar(j2).le.6.or.kstar(j2).eq.9)then
         CALL comenv(mass0(j2),mass(j2),massc(j2),aj(j2),jspin(j2),
     &               kstar(j2),mass0(j1),mass(j1),massc(j1),aj(j1),
     &               jspin(j1),kstar(j1),z,zpars,ecc,sep,jorb,coel)
         com = .true.
      else
         CALL mix(mass0,mass,aj,kstar,zpars)
      endif
