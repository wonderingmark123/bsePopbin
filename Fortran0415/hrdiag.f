...
!     ADDED
!
      real*8 rc_b_hehg          ! Core radius at the beginning of HeHG phase
      real*8 pisnf              ! PISNE & PPISNE factor
!
!----------------------------------------------------------------------
*
*
...
*
     ...
     &(psflag.gt.1.and.mcbagb.gt.65.d0.and.mcbagb.le.135.d0))then
*
...
*
                  ...
                  lum = 1.0d-10
                  r = 1.0d-10
       elseif(psflag.gt.1.and.mcbagb.ge.45.d0.and.mcbagb.le.65.d0)then
...
*
                  ...
                  if((mass.lt.1.6d0).or.
     &(psflag.gt.1.and.mass.gt.65.d0.and.mass.le.135.d0))then
*
* Star is not massive enough to ignite C burning.
* so no remnant is left after the SN
*               OR
...
!----------------------------------------------------------------------
!     EDITED according to:
!     Hall P. D., Tout C. A., 2014, MNRAS, 444, 3209,
!     Section 3.1.1 & Appendix A (Table A1): Low-mass HG and RGB
!     (zpars(15+i) == c_i) in eqn.A1
!
            rc = rx * (zpars(15)
     &           + mc*(zpars(16)
     &           + mc*(zpars(17)
...
!     Hall P. D., Tout C. A., 2014, MNRAS, 444, 3209
!----------------------------------------------------------------------
!     Section 3.2 & Appendix A (Table A2): TPAGB
!
         if(kw.eq.6)then
            rc = rx * (+7.342054d+0
     &           + mc*(-1.328317d+1
...
!----------------------------------------------------------------------
!     Section 3.3.1: HeHG
!     Core radius is fixed at the beginning of the HeHG phase.
!     Further improvement is likely to require a fit including a dependence
!     on total mass.
!
         elseif(kw.eq.8)then
            if(isnan(rc_b_hehg))then
               rc_b_hehg = (1.23d-3 + mc*(8.06d-2 - 3.31d-3*mc)) /
     &              (1.d0 + mc*(0.467d0 - 3.03d-2*mc))
            endif
            rc = rc_b_hehg
!
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
!     ADDED accroding to:
!     Spera M., Mapelli M., 2017, MNRAS, 470, 4739, and SVEN:
!     (src/base/Stellarevolution.cpp: Stellarevolution::pair_instability)
!----------------------------------------------------------------------
!
      real*8 FUNCTION pisnf(mt,kw,mc)
      implicit none
      integer kw
      real*8 mt,mc
      real*8 mhe,f,k,s
      ...
      f = mhe/mt
      k = 0.67d0*f + 0.1d0
      s = 0.5226d0*f - 0.52974d0

      if(mhe.le.32.0d0)then
         pisnf = 1.0d0
      elseif(mhe.ge.135.0d0)then
         pisnf = 1.0d0
      elseif(mhe.ge.64.0d0)then
         pisnf = 0.0d0
      else
         if(f.lt.0.9d0)then
            if(mhe.le.37.d0)then
               pisnf = 0.2d0*(k-1.d0)*mhe + 0.2d0*(37.d0-32.d0*k)
            elseif(mhe.le.60.d0)then
               pisnf = k
            else
               pisnf = -0.25d0*k*mhe + 16.d0*k
            endif
         else
            if(mhe.le.37.d0)then
               pisnf = s*(mhe-32.d0) + 1.d0
            elseif(mhe.le.56.d0)then
               pisnf = 5.d0*s + 1.d0
               if(pisnf.ge.0.82916d0)then
                  pisnf = (-0.1381d0*f+0.1309d0)*(mhe-56.d0) + 0.82916d0
               endif
            else
               pisnf = -0.103645d0*mhe + 6.63328d0
            endif
         endif
      endif

      if(pisnf.gt.1.d0)then
         pisnf = 1.d0
         write(0,*)'pisnf = ',pisnf
      elseif(pisnf.lt.0.d0)then
         pisnf = 0.d0
         write(0,*)'pisnf = ',pisnf
      endif
